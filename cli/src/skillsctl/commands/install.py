...
def _target_file(
    ...
    if custom_path is not None:
        target_dir = (project_root / custom_path).resolve()
    else:
...
@click.option(
    ...
    metavar="DIR",
    help=(
        "Directory to write files into (e.g. .claude/commands). "
        "Files are placed flat as {name}.md — no category subfolder is added. "
        "Dependencies always use the default .skills/{category}/ location."
    ),
)
@click.pass_context
def install(
    ...
    installed: list[tuple[str, str, str, Path, bool]] = []  # (name, version, category, path, is_dep)
    ...
    for name, item_meta in resolved.items():
        ...
        # Dependencies always use default location; only explicit installs get --path
        effective_path = None if is_dep else install_path
        target_file = _target_file(project_root, name, category, effective_path)
